...
var mailboxForwardingAddCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		err = server.MailboxAddForwarding(mailboxAddress, destinationAddress)
		if err != nil {
			color.Red(err.Error())
			os.Exit(1)
		}

		success("Successfully added mailbox-forwarding %v -> %v\n", mailboxAddress, destinationAddress)
	},
}
...
func init() {
	...
	mailboxForwardingAddCmd.SetUsageTemplate(`Usage:{{if .Runnable}}
  iredmail-cli mailbox add-forwarding [mailbox address] [destination address]{{end}}{{if .HasAvailableSubCommands}}
